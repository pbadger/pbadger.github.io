// ros_webrtc from https://github.com/mayfieldrobotics/ros-webrtc, with some changes to support mute and unmute.
...
ros_webrtc.Call = Backbone.Model.extend({
  ...
  _pickup: function(signaling, session_id) {
    ...
    var that = this;
    var createAndSendOffer = function(pc, that){
      pc.createOffer(
         function (offer) {
    ...
    // navigator.getUserMedia({ audio: true, video: false },
      // function(stream) {
        ...
        createAndSendOffer(that.pc, that);
      // },
      // function (err) {
  ...
  _addIceCandidate: function (signaling, session_id, ice_candidate) {
    ...
    var that = this;
    if (!ice_candidate.candidate.includes('192.168')) { // use this code to force webrtc traffic over the vpn 
      this.pc.addIceCandidate(
        ice_candidate,
        function () {
          console.info( 'pc added remote ice candidate', ice_candidate);
  ...
  _pcIceCandidate: function (event) {
    ...
    if (!event.candidate.candidate.includes('192.168')) { // use this code to force webrtc traffic over the vpn.
